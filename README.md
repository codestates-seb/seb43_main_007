...
|<닉네임 변경> <br /> - 닉네임 유효성 검사 통과 시 저장 버튼이 활성화된다. <br /> - 닉네임 중복 시 변경 실패 알림이 뜬다. <br /> <프로필 사진 변경> <br /> - JPG, PNG 1MB 미만 파일 업로드 시 프로필 사진이 변경된다 <br /> - 삭제 버튼 클릭 시 기본 프로필 사진으로 변경된다. <br />  <비밀번호 변경> <br /> - 새로운 비밀번호 유효성 검사 및 재입력이 일치할 경우 변경버튼이 활성화된다. <br /> - 현재 비밀번호가 일치하지 않을 시 변경 실패 알림이 뜬다. <br />  <계정 삭제> <br /> - 계정 삭제 버튼 클릭 시 모달창이 뜬다. <br /> - 모달창 내 계정삭제 버튼 클릭 시 계정이 삭제 되고 홈화면으로 이동한다. |<북마크> <br /> -내가 북마크한 게시글을 카드형태로 볼 수 있다. <br /> - 카드를 클릭하면 해당 게시글로 이동한다.<br /> |내가 쓴 글 <br /> - 내가 쓴 글과 내가 쓴 댓글 목록이 나열된다. <br /> - 제목과 댓글을 클릭시 본인이 작성한 상세페이지로 이동한다.
